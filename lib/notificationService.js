 ...
 */
export async function sendNotification(type, data, recipients) {
  try {
 ...
 */
export async function notifyCreditPurchase(studentEmail, studentName, creditsPurchased, amount, paymentMethod) {
  const { getUserEmailsByRole } = await import('./notifications');
  const adminEmails = await getUserEmailsByRole(['admin', 'superadmin']);
