...
export async function handleNoShow(sessionId, noShowType) {
  try {
    ...
    // If student no-show, forfeit credits
    if (noShowType === "student-no-show") {
      // Credits are already deducted, so no additional action needed
      // But we could log this for analytics
      return {
        success: true,
        message: "Student no-show recorded. Credits forfeited.",
      };
    }

    // If tutor no-show, refund credits to student
    if (noShowType === "tutor-no-show") {
      ...
      const newCredits = (student.credits || 0) + session.credits_required;
      ...
      await supabase
        .from("Schedules")
        .update({ credits_refunded: session.credits_required })
        .eq("id", sessionId);

      return {
        success: true,
        message: "Tutor no-show recorded. Credits refunded to student.",
        creditsRefunded: session.credits_required,
      };
    }
  } catch (error) {
